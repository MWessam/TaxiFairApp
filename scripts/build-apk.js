...
async function buildAPK(versionType = 'patch', skipVersionUpdate = false, forceClean = false, runPrebuild = false) {
  ...
  try {
    ...
    if (fs.existsSync(apkPath)) {
      ...
      const outputPath = path.join(outputDir, `taxifairmock-${versionInfo?.version || 'unknown'}.apk`);
...
if (require.main === module) {
  ...
  buildAPK(versionType, skipVersionUpdate, forceClean, runPrebuild);
}
